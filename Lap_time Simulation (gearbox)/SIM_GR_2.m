...
plot(s, S_best,'LineWidth',1.2,'Color',[0.1 0.7 0.2]);      % drive
...
leg2.FontSize = 25; set(findall(leg2,'Type','text'),'Color','k');

exportgraphics(fig, fullfile(output_folder,'speed_forces.pdf'),'ContentType','vector');

%% =========================================================
% POWER (ENERGY SEMANTIC COLOR)
% =========================================================
...
exportgraphics(fig, fullfile(output_folder,'power_profile.pdf'),'ContentType','vector');
...
yq = linspace(min(gear_ratio_planetary_first), max(gear_ratio_planetary_first), 175);
[Xq,Yq] = meshgrid(xq,yq);

Zq = interp2(X,Y,lap_time_vec,Xq,Yq,'linear');
...
clim([141.5 143.5]);   % ✔ alinhado com tudo

cb = colorbar;
cb.Color='k'; cb.FontName='Helvetica'; cb.FontSize=25;
