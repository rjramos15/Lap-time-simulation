...
% =========================================================
precision = 20;
gear_ratio_planetary_first = linspace(1.0, 2.0, precision);
gear_ratio_planetary_second = linspace(1.0, 2.0, precision);
...
% =========================================================
%torque = interp1(rt, t, 1:maxrpm,'pchip', NaN);
for kkk = 1:precision
for kk = 1:precision
for k = 1:precision

    % i_total = gear_ratio_vec(k);
    i_first = gear_ratio_planetary_first(kkk) * gear_ratio_final(k);
    i_second = gear_ratio_planetary_second(kk) * gear_ratio_final(k);
    ...
    for i = 1:N
        ...
        if rpm_third > maxrpm, torque_third = 0;
        else, torque_third = interp1(rt, t, rpm_third, 'pchip', 0); end
    ...
    for it = 1:nIter
        ...
        for ii = 0:N-1
            ...
            if F_lat > F_max
                error('Warning: backward | F_lat > F_max at i = %d\n', next);
            end
    ...
    if lap_time < best_lap_time
        ...
        best_gear_planetary_first = gear_ratio_planetary_first(kkk);
        best_gear_planetary_second = gear_ratio_planetary_second(kk);
...
fprintf('Best Planetary Ratio (2nd): %.4f\n', best_gear_planetary_second);
...
%% =========================================================
% PLOTS (THREE-SPEED — STANDARDIZED, CORRECTED)
% =========================================================
...
plot(s, S_best,       'LineWidth',1.2,'Color',[0.1 0.7 0.2]);
plot(s, F_drag_best,  'LineWidth',1.0,'Color',[0.0 0.45 0.75]);
plot(s, F_rr_best,    'LineWidth',1.0,'Color',[0.75 0.2 0.75]);
plot(s, F_slope_best, 'LineWidth',1.0,'Color',[0.9 0.6 0.1]);

grid on
ax2.XGrid = 'off';
ax2.GridColor = [0.82 0.82 0.82];
ax2.GridAlpha = 0.35;
...
leg2.TextColor = [0 0 0];
leg2.EdgeColor = 'none';
set(leg2,'TextColor',[0 0 0]);
...
%% =========================================================
% PERFORMANCE MAP — FINAL RATIO VS PLANETARY 1
% =========================================================

Z_map1 = squeeze(min(lap_time_vec, [], 2));

[X1, Y1] = meshgrid(gear_ratio_final, gear_ratio_planetary_first);

xq1 = linspace(min(gear_ratio_final), max(gear_ratio_final), 175);
yq1 = linspace(min(gear_ratio_planetary_first), max(gear_ratio_planetary_first), 175);
[Xq1, Yq1] = meshgrid(xq1, yq1);

Zq1 = interp2(X1, Y1, Z_map1, Xq1, Yq1, 'linear');

fig = figure('Name','Final vs Planetary 1','Color','w');
ax = gca; hold on
set(ax,'Color','w','FontName','Helvetica','FontSize',25,...
    'LineWidth',1,'XColor','k','YColor','k')
...
plot(best_gear_final, best_gear_planetary_first,...
...
% garantir que Y não fica "esticado"
ylim([min(gear_ratio_planetary_first) max(gear_ratio_planetary_first)])

exportgraphics(fig, fullfile(output_folder,'gr_three1.pdf'),...
    'ContentType','vector','BackgroundColor','white');

%% =========================================================
% PERFORMANCE MAP — FINAL RATIO VS PLANETARY 2
% =========================================================

Z_map2 = squeeze(min(lap_time_vec, [], 1));

[X2, Y2] = meshgrid(gear_ratio_final, gear_ratio_planetary_second);

xq2 = linspace(min(gear_ratio_final), max(gear_ratio_final), 175);
yq2 = linspace(min(gear_ratio_planetary_second), max(gear_ratio_planetary_second), 175);
[Xq2, Yq2] = meshgrid(xq2, yq2);

Zq2 = interp2(X2, Y2, Z_map2, Xq2, Yq2, 'linear');

fig = figure('Name','Final vs Planetary 2','Color','w');
ax = gca; hold on
set(ax,'Color','w','FontName','Helvetica','FontSize',25,...
    'LineWidth',1,'XColor','k','YColor','k')
...
plot(best_gear_final, best_gear_planetary_second,...
    'ko','MarkerSize',6,'MarkerFaceColor','w','LineWidth',1);
...
% garantir que Y não fica "esticado"
ylim([min(gear_ratio_planetary_second) max(gear_ratio_planetary_second)])
