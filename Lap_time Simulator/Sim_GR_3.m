...
% t  = PCdatasheet.Torque1(:);       %Comment to test other curves
% rt = PCdatasheet.Speed1(:);                   
...
% =========================================================
rpm_step = 0.1;
rpm_grid = (0:rpm_step:maxrpm)';
torque_grid = interp1(rt, t, rpm_grid, 'pchip', 0);

%% =========================================================
% LOCAL SPEED LIMIT FROM CURVATURE
% =========================================================
denom_cl = ( m ./ abs(R) + mu * 0.5 * rho * frontA * Cl );
denom_cl(denom_cl<0) = 0;
v_lim_curve = sqrt(mu * m * g .* cos(theta) ./ denom_cl);
...
% =========================================================
precision = 25;
gear_ratio_ratio_first = linspace(1.0, 2.0, precision);
gear_ratio_ratio_second = linspace(1.0, 2.0, precision);
gear_ratio_final = linspace(3.0, 4.0, precision);
i_current = zeros(1,length(s));

lap_time_vec   = inf(precision, precision, precision);
...
%Progress bar
h = waitbar(0, 'Iniciando processamento...');
barcount = 0;
...
for k = 1:precision
    ...
    i_first = gear_ratio_ratio_first(kkk) * gear_ratio_final(k);
    i_second = gear_ratio_ratio_second(kk) * gear_ratio_final(k);
    i_third = gear_ratio_final(k);
    i_current(:) = i_second;
    
    v_redline = maxrpm*(2*pi*r_wheel)/(60*i_third);
    v = v_lim_curve;
    ...
    for ii = 0:N-1
        ...
        F_down = - 0.5 * rho * frontA * Cl * v(next)^2;

        F_max = mu * (m * g * cos(theta(next)) + F_down);
    ...
    for i = 1:N
        ...
        F_down = - 0.5 * rho * frontA * Cl * v(i)^2;

        F_max = mu * (m * g * cos(theta(i)) + F_down) * (wb - cgx) / wb;
        ...
        rpm_third = v(i) * i_third * 60 / (r_wheel *2*pi);

        idx_rpm_first = round(rpm_first/rpm_step) + 1;

        if idx_rpm_first >= 1 && idx_rpm_first <= numel(torque_grid)
            torque_first = torque_grid(idx_rpm_first);
        else
            torque_first = 0;
        end
        
        idx_rpm_second = round(rpm_second/rpm_step) + 1;

        if idx_rpm_second >= 1 && idx_rpm_second <= numel(torque_grid)
            torque_second = torque_grid(idx_rpm_second);
        else
            torque_second = 0;
        end

        idx_rpm_third = round(rpm_third/rpm_step) + 1;

        if idx_rpm_third >= 1 && idx_rpm_third <= numel(torque_grid)
            torque_third = torque_grid(idx_rpm_third);
        else
            torque_third = 0;
        end
        ...
        F_third = min(torque_third * i_third * eta / r_wheel, F_long_tyre);

        if (i_current(i) == i_first)
            if (F_second > F_first * 1.00 || rpm_first > maxrpm) && (rpm_second <= maxrpm)
        ...
        elseif (i_current(i) == i_second)
            if (F_third > F_second * 1.00 || rpm_second > maxrpm)
                i_current(i) = i_third;
                F_motor = F_third;
            elseif (F_first > F_second * 1.00 && rpm_first < maxrpm)
                i_current(i) = i_first;
                F_motor = F_first;
            else
                F_motor = F_second;
        ...
        elseif (i_current(i) == i_third)
            if (F_second > F_third * 1.00 && rpm_second < maxrpm)
                i_current(i) = i_second;
                F_motor = F_second;
            else
                F_motor = F_third;
    ...
    for it = 1:nIter
        ...
        for ii = 0:N-1
            ...
            Nr = max(m*g*cos(theta(next))*(wb-cgx)/wb + m*a_local*cgh/wb ...
                + F_down*(wb-cgx)/wb, 0);
            Nf = max(m*g*cos(theta(next))*cgx/wb - m*a_local*cgh/wb ...
                + F_down*cgx/wb, 0);

            F_max_f = mu * Nf;
            F_max_r = mu * Nr;
            F_lat_f = min((m * v(next)^2 / abs(R(next))) * Nf/(Nf+Nr), F_max_f);
            F_lat_r = min((m * v(next)^2 / abs(R(next))) * Nr/(Nf+Nr), F_max_r);

            F_long_tyre = sqrt(F_max_f^2 - F_lat_f^2) + sqrt(F_max_r^2 - F_lat_r^2);
        ...
        for i = 1:N
            ...
            Nr = max(m*g*cos(theta(i))*(wb-cgx)/wb + m*a_local*cgh/wb ...
                + F_down*(wb-cgx)/wb, 0);
            Nf = max(m*g*cos(theta(i))*cgx/wb - m*a_local*cgh/wb ...
                + F_down*cgx/wb, 0);

            F_max = max(mu * Nr, 0);
            F_lat = min((m * v(i)^2 / abs(R(i))) * Nr/(Nf+Nr), F_max);
            ...
            elseif (i_current(i) == i_third)
                if (F_second > F_third * 1.0 && rpm_second < maxrpm)
                    i_current(i) = i_second;
                    F_motor = F_second;
                else
                    F_motor = F_third;
    ...
    lap_time = sum(dt);
    lap_time_vec(kkk, kk, k) = lap_time;

    if lap_time < best_lap_time
        best_lap_time   = lap_time;
        best_v          = v;
        best_v_lim      = v_lim_curve;
        best_gear_ratio_first = gear_ratio_ratio_first(kkk);
        best_gear_ratio_second = gear_ratio_ratio_second(kk);
        ...
        best_perc_3 = round(sum(best_ratio_vec == i_third) / N * 100, 2);
        best_dt = dt;
    end

end
    barcount = barcount + 1;
    perc = barcount / precision^2;
...
best_force_available = zeros(N,1);
F_lat_best   = zeros(N,1);
F_drag_best  = zeros(N,1);
F_down_best  = zeros(N,1);
F_rr_best    = zeros(N,1);
...
for i = 1:N
    ...
    F_down_best(i) = - 0.5 * rho * frontA * Cl * v_best(i)^2;

    Nr = max(m*g*cos(theta(i))*(wb-cgx)/wb + m*ax_best(i)*cgh/wb ...
            + F_down_best(i)*(wb-cgx)/wb, 0);
    Nf = max(m*g*cos(theta(i))*cgx/wb - m*ax_best(i)*cgh/wb ...
        + F_down_best(i)*cgx/wb, 0);

    if ax_best(i) < 0
        F_max_f = mu * Nf;
        F_max_r = mu * Nr;
        F_lat_f = min((m * v_best(i)^2 / abs(R(i))) * Nf/(Nf+Nr), F_max_f);
        F_lat_r = min((m * v_best(i)^2 / abs(R(i))) * Nr/(Nf+Nr), F_max_r);
        F_long_tyre = sqrt(F_max_f^2 - F_lat_f^2) + sqrt(F_max_r^2 - F_lat_r^2);
        F_max_best(i) = F_max_r;    
        F_lat_best(i) = F_lat_r;    
    else
        F_max = max(mu * Nr, 0);
        F_lat = min((m * v_best(i)^2 / abs(R(i))) * Nr/(Nf+Nr), F_max);
        F_long_tyre = sqrt(F_max^2 - F_lat^2);
        F_max_best(i) = F_max;      
        F_lat_best(i) = F_lat;      
    end

    best_force_available(i) = F_long_tyre;
    ...
    if idx_rpm >= 1 && idx_rpm <= numel(torque_grid)
        torquemotor = torque_grid(idx_rpm);
    else
        torquemotor = 0;
    end
...
fprintf('   3rd Gear: %.2f%%\n', best_perc_3);
fprintf('-----------------------------------------------------------------------\n');
fprintf('Best Ratio (1st): %.4f\n', best_gear_ratio_first);
fprintf('Best Ratio (2nd): %.4f\n', best_gear_ratio_second);
fprintf('Best Final Ratio     (3rd):        %.4f\n', best_gear_final);
fprintf('-----------------------------------------------------------------------\n');
...
P_inst = S_best .* v_best;           % [W] wheel power
...
output_folder = 'three_speed_plots';
...
set(ax1,'Color','w','FontName','Times','FontSize',25,...
...
leg1.FontSize = 25;
leg1.FontName = 'Times';
leg1.TextColor = [0 0 0];
leg1.EdgeColor = 'none';
set(leg1,'TextColor',[0 0 0]);
set(findall(leg1,'Type','text'),'Color',[0 0 0]);

subplot(2,1,2)
ax2 = gca; hold(ax2,'on'); box(ax2,'on');
set(ax2,'Color','w','FontName','Times','FontSize',25,...
    'LineWidth',1,'XColor','k','YColor','k');

plot(s, min(best_force_available, 1500), '--', 'LineWidth', 0.8, 'Color', [0.2 0.2 0.2]);
plot(s, S_best,        'LineWidth',1.1,'Color',[0.1 0.7 0.2]);   % drive
plot(s, F_drag_best,   'LineWidth',1.0,'Color',[0.0 0.45 0.75]); % drag
plot(s, F_down_best, 'LineWidth', 1.0, 'Color', [0.0 0.25 0.45]); % down
...
s_accumulated = [0; cumsum(ds_vec(1:end-1))];
E_battery_plot_kWh = 6.5;
E_cum_J = cumsum(E_seg);              % [J]
E_battery_J = E_battery_plot_kWh * 3.6e6;  % [J]
...
fig = figure('Name','State of Charge Profile','Color','w');
ax = axes(fig); hold(ax,'on'); box(ax,'on');

set(ax,'Color','w','FontName','Times','FontSize',25,...
    'LineWidth',1,'XColor','k','YColor','k');

plot(s_accumulated, SOC,...
    'LineWidth',2.0,...
...
ax.YLim = [min(SOC) 100];
yticks(round(linspace(ceil(min(SOC)*10)/10, 100, 5),1))

xlabel('s [m]','FontSize',30,'FontName','Times');
ylabel('SOC [%]','FontSize',30,'FontName','Times');

leg = legend(sprintf('State of Charge for %.1f kWh capacity', round(E_battery_plot_kWh,1)),...
...
exportgraphics(fig, fullfile(output_folder,'soc_profile.pdf'),...
    'ContentType','vector','BackgroundColor','white');
...
plot(s, best_ax,'LineWidth',1.2,'Color',[0.85 0.1 0.1]);
plot(s, best_ay,'LineWidth',1.2,'Color',[0.1 0.35 0.85]);
...
%% =========================================================
% PERFORMANCE MAP (FINAL RATIO VS FIRST STAGE)
% =========================================================

Z_map1 = squeeze(min(lap_time_vec, [], 2));

[X1, Y1] = meshgrid(gear_ratio_final, gear_ratio_ratio_first);

xq1 = linspace(min(gear_ratio_final), max(gear_ratio_final), 175);
yq1 = linspace(min(gear_ratio_ratio_first), max(gear_ratio_ratio_first), 175);
[Xq1, Yq1] = meshgrid(xq1, yq1);

Zq1 = interp2(X1, Y1, Z_map1, Xq1, Yq1, 'pchip');

fig = figure('Name','i_f vs i_1', 'Color','w');
ax = gca; hold on
set(ax,'Color','w','FontName','Times','FontSize',25,...
    'LineWidth',1,'XColor','k','YColor','k')

contourf(Xq1, Yq1, Zq1, 35, 'LineStyle','none');
colormap(turbo)

contour(Xq1, Yq1, Zq1, 14,...
...
cb.Ticks = linspace(auxtime(1), auxtime(2), 4);

xlabel('i_f','FontSize',30,'FontName','Times');
ylabel('i_1','FontSize',30,'FontName','Times');

plot(best_gear_final, best_gear_ratio_first,...
    'ko','MarkerSize',6,'MarkerFaceColor','w','LineWidth',1);
...
ylim([min(gear_ratio_ratio_first) max(gear_ratio_ratio_first)])

exportgraphics(fig, fullfile(output_folder,'gr_three1.pdf'),...
    'ContentType','vector','BackgroundColor','white');

%% =========================================================
% PERFORMANCE MAP (FINAL RATIO VS SECOND STAGE)
% =========================================================

Z_map2 = squeeze(min(lap_time_vec, [], 1));

[X2, Y2] = meshgrid(gear_ratio_final, gear_ratio_ratio_second);

xq2 = linspace(min(gear_ratio_final), max(gear_ratio_final), 175);
yq2 = linspace(min(gear_ratio_ratio_second), max(gear_ratio_ratio_second), 175);
[Xq2, Yq2] = meshgrid(xq2, yq2);

Zq2 = interp2(X2, Y2, Z_map2, Xq2, Yq2, 'pchip');
...
contourf(Xq2, Yq2, Zq2, 35, 'LineStyle','none');
colormap(turbo)

contour(Xq2, Yq2, Zq2, 14,...
    'LineColor',[0.2 0.2 0.2],...
    'LineWidth',0.5);
...
xlabel('i_f','FontSize',30,'FontName','Times');
ylabel('i_2','FontSize',30,'FontName','Times');

plot(best_gear_final, best_gear_ratio_second,...
    'ko','MarkerSize',6,'MarkerFaceColor','w','LineWidth',1);
...
ylim([min(gear_ratio_ratio_second) max(gear_ratio_ratio_second)])

exportgraphics(fig, fullfile(output_folder,'gr_three2.pdf'),...
    'ContentType','vector','BackgroundColor','white');
...
V = smoothdata(V,1,'sgolay',7);
V = smoothdata(V,2,'sgolay',7);
...
exportgraphics(fig, fullfile(output_folder,'ggv_three.pdf'),...
    'ContentType','vector','BackgroundColor','white');
...
function [xpos, R, theta] = distance(lat, lon, xr, r, pos, rad, ds)
    ...
    kappa = smoothdata(kappa, 'sgolay', 5/ds);
    theta = smoothdata(theta, 'sgolay', 5/ds);
