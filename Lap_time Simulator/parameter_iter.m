...
parameter = linspace(170, 270, precision);
...
for k = 1:precision
    ...
    for ii = 0:N-1
        ...
        a_stoppie     = max(g * (cos(theta(next)) * (wb - cgx) / cgh ...
            + sin(theta(next))) + F_drag / m + F_rr / m ...
            + F_down * (wb - cgx) / (m * cgh), 0);
...
[X,Y] = meshgrid(gear_ratio_vec, parameter);

xq = linspace(min(gear_ratio_vec), max(gear_ratio_vec), 175);
yq = linspace(min(parameter), max(parameter), 175);
[Xq,Yq] = meshgrid(xq,yq);

Zq = interp2(X,Y,lap_time_vec,Xq,Yq,'pchip');

fig = figure('Name','Parameter Map','Color','w');
ax = gca; hold on
...
cb = colorbar;
cb.Color='k'; cb.FontName='Times'; cb.FontSize=25; cb.Ticks = linspace(138,144,7);
cb.Label.String='Lap Time [s]'; cb.Label.FontName='Times'; cb.Label.FontSize=30;

xlabel('i_f','FontSize',30,'FontName','Times');
ylabel('m [kg]','FontSize',30,'FontName','Times');

if exist('best_gear_ratio','var')
    plot(best_gear_ratio,best_parameter,...
...
xlim([3 4])
xticks([3 3.5 4])
yticks(linspace(170, 270, 6))

ylim([min(parameter) max(parameter)])

exportgraphics(fig, fullfile(output_folder,'m_it.pdf'),'ContentType','vector');
