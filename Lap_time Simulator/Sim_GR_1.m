...
% =========================================================
precision = 100;
gear_ratio_vec = linspace(3, 4, precision);
...
for k = 1:length(gear_ratio_vec)
    ...
    waitbar(perc, h, sprintf('Progress: %d%%', round(perc*100)));
end
...
cb.Label.Color = 'k';
cb.Ticks = linspace(auxtime(1), auxtime(2), 3);

clim(auxtime)
ylim(auxtime)
yticks(linspace(auxtime(1), auxtime(2), 3))
