...
for k = 1:precision
    ...
    i_first = gear_ratio_ratio_first(kk) * gear_ratio_final(k);
    ...
    if lap_time < best_lap_time
        ...
        best_gear_ratio_first = gear_ratio_ratio_first(kk);
...
%% =========================================================
% PLOTS (STANDARDIZED - PAPER READY)
% =========================================================

output_folder = 'two_speed_plots';
...
leg1.FontSize = 25; set(findall(leg1,'Type','text'),'Color','k');
...
ax.YLim = [min(SOC) 100];
yticks(round(linspace(ceil(min(SOC)*10)/10, 100, 6),1))
...
leg.FontSize = 25; set(findall(leg,'Type','text'),'Color','k');

exportgraphics(fig, fullfile(output_folder,'latlon_acceleration.pdf'),'ContentType','vector');

%% =========================================================
% PERFORMANCE MAP (CONSISTENT SCALE)
% =========================================================

[X,Y] = meshgrid(gear_ratio_final, gear_ratio_ratio_first);

xq = linspace(min(gear_ratio_final), max(gear_ratio_final), 175);
yq = linspace(min(gear_ratio_ratio_first), max(gear_ratio_ratio_first), 175);
[Xq,Yq] = meshgrid(xq,yq);

Zq = interp2(X,Y,lap_time_vec,Xq,Yq,'pchip');

fig = figure('Name','Performance Map','Color','w');
ax = gca; hold on
...
exportgraphics(fig, fullfile(output_folder,'gr_two.pdf'),'ContentType','vector');
...
F = scatteredInterpolant(best_ay,best_ax,v_best,'natural','nearest');
V = F(AY,AX);
...
colormap(turbo)
contour(AY,AX,V,16,'LineColor',[0.2 0.2 0.2],'LineWidth',0.5);

clim([min(v_best) max(v_best)])

cb = colorbar;
cb.Color='k'; cb.FontName='Times'; cb.FontSize=25;
cb.Label.String='Velocity [m/s]'; cb.Label.FontName='Times'; cb.Label.FontSize=25;
...
exportgraphics(fig, fullfile(output_folder,'ggv_two.pdf'),'ContentType','vector');
