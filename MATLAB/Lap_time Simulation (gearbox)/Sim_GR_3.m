...
% =========================================================
precision = 5;
gear_ratio_planetary_first = linspace(1.460, 1.465, precision);
gear_ratio_planetary_second = linspace(1.175 ,1.190, precision);
gear_ratio_final = linspace(3.570, 3.590, precision);
...
%% Surface de Combinação Ótima no Planetário
[~, idx_final] = min(abs(gear_ratio_final - best_gear_final));

z_data = squeeze(lap_time_vec(:, :, idx_final));

[X, Y] = meshgrid(gear_ratio_planetary_second, gear_ratio_planetary_first);

figure('Name', 'Performance Map');
surf(X, Y, z_data); % Se der erro de dimensão, tente surf(X, Y, z_data')
shading interp; % Deixa a superfície lisa
colorbar;
xlabel('Planetary (2nd Gear)');
ylabel('Planetary (1nd Gear)');
zlabel('Lap Time [s]');
title(['Performance Map (Final Ratio fixed at: ', num2str(best_gear_final), ')']);
hold on;

% Marcar o ponto ótimo na superfície
plot3(best_gear_planetary_second, best_gear_planetary_first, best_lap_time, ...
