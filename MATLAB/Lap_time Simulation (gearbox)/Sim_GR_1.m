...
% =========================================================
precision = 50;
gear_ratio_vec = linspace(3.80, 4.00, precision);
...
title(sprintf('Speed profile | best gear ratio = %.4f | lap time = %.3f s', ...
    best_gear_ratio, best_lap_time));
...
figure('Name', 'Lateral and Longitudinal Acceleration');
...
title('Lateral and Longitudinal Acceleration Profile');

%% Gear Ratio vs Lap-Time

figure('Name', 'Gear-Ratio Influence on Lap-Time');
plot(gear_ratio_vec, lap_time_vec, 'LineWidth', 1.0);
grid on;
xlabel('Gear ratio');
ylabel('Lap time [s]');
...
plot(ay_env, ax_env_pos, '--', 'LineWidth', 1.0);
plot(ay_env, ax_env_neg, '--', 'LineWidth', 1.0);
